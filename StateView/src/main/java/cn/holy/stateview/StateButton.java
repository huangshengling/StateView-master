...
public class StateButton extends Button {
    ...
    private boolean mCenterImageText;
    ...
    private void setup(AttributeSet attrs) {
        ...
        mCenterImageText = a.getBoolean(R.styleable.StateButton_centerImageText, false);
    ...
    protected void onDraw(Canvas canvas) {
        if (mCenterImageText) {
            ...
            if (drawables != null) {
                ...
                float leftBodyWidth = getBodyWidth(leftDrawable);
                Drawable rightDrawable = drawables[2];
                float rightBodyWidth = getBodyWidth(rightDrawable);
                float totalWidth = leftBodyWidth + rightBodyWidth + textWidth;
                if (totalWidth <= getWidth()) {
                    canvas.translate((getWidth() - totalWidth) / 2, 0);
                    setPadding(getPaddingLeft(), getPaddingTop(), (int) (getWidth() - totalWidth), getPaddingBottom());
                }
            }
        }
        super.onDraw(canvas);
